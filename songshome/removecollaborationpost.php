...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST["collaborationId"]) && isset($_POST["collaborationKey"]) && isset($_POST["removePostId"])) {
        ...
        if ($collaborationQuery) {
            ...
            if ($songQuery) {
                ...
                if (isset($_SESSION['member_id'])) { // User is logged in.
                    ...
                    $stmt = $db->prepare("SELECT * FROM `collaboration_posts` WHERE `collaboration_id` = :collaborationid AND `id` = :postid");
                    $stmt->bindParam('collaborationid',  $collaborationQuery["id"]);
                    $stmt->bindParam('postid',  $_POST["removePostId"]);
                    $stmt->execute();
                    ...
                    if ($userInCollaborationQuery && ($collaborationQuery["created_by_member_id"] == $_SESSION['member_id'] || $_SESSION['member_id'] == $targetPostQuery["member_id"] || $access_level >= 5)) {

                        $allDeleted = 1; 
                        if($targetPostQuery["filename"] != "" && file_exists("./collaborationPostFiles/" . $targetPostQuery["filename"])) {
                            if(!unlink("./collaborationPostFiles/" . $targetPostQuery["filename"])) { // Remove post file.
                        ...
                        if  ($allDeleted) { // If all files were deleted, remove collaboration from the database.
                            try {                             
                                $stmt = $db->prepare("DELETE FROM `collaboration_posts` WHERE `collaboration_id` = :collaborationid AND `id` = :postid");
                                $stmt->bindParam('collaborationid',  $collaborationQuery["id"]);
                                $stmt->bindParam('postid',  $_POST["removePostId"]);
                                $stmt->execute();
